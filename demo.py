...
def hotword():
    # Load the pocketsphinx models
    model_path = get_model_path()
    
    # Define keywords and their sensitivity
    keywords = [('wisdom', 1e-20), ('jarvis', 1e-20)]

    try:
        # Initialize LiveSpeech for real-time speech processing
        speech = LiveSpeech(
            verbose=False,
            sampling_rate=16000,
            buffer_size=2048,
            no_search=False,
            full_utt=False,
            hmm=os.path.join(model_path, 'en-us-8k'),
            lm=False,
            dic=os.path.join(model_path, 'en-us'),
            kws_threshold=1e-10,
            kws=keywords
        )
        ...
        # Loop to process audio frames and detect keywords
